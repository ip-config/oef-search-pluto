...
if __name__ == "__main__":
    ...
    search_network = SearchNetwork.SearchNetwork({'0': com}, [
        'search-0', 'search-1' 'search-2' 'search-3' 'search-4'
        ])

    # Search connectivity
    search_network.set_connection('search-0', ['1', '4'])
    search_network.set_connection('search-1', ['2', '3'])
    search_network.set_connection('search-2', ['3', '4'])
    search_network.set_connection('search-3', ['4', '0'])
    search_network.set_connection('search-4', ['1', '2'])

    connection_factory = ConnectionFactory.ConnectionFactory()

    oef1 = search_network.create_oef_core_for_node('search-0', "oef1", connection_factory)
